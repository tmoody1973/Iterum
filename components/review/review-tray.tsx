...
import { CreativeRouteCard } from './creative-route-card'
...
export function ReviewTray({ snapshot, runtime, onClose, proposalPlacement }: { snapshot: WorkspaceState; runtime: WorkspaceRuntime; onClose?: () => void; proposalPlacement?: Point }) {
  ...
  const pendingRoutes = snapshot.creativeRoutes.filter((route) => route.status === 'pending')
  ...
  const layoutDecision = (proposalId: string, decision: 'approve' | 'reject') => {
    ...
    if (result.ok) setPreviewLayoutProposal(null)
  }
  return <aside className="review-tray" id="review-tray" aria-label="Review tray">
    <div className="review-heading"><h2>Review tray</h2><span>{pending.length + pendingType.length + pendingLayouts.length + pendingRoutes.length} pending</span><button className="drawer-close" type="button" onClick={onClose}>Board preview</button></div>
