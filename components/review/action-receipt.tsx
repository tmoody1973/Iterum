...
export function ActionReceipt({ receipt, runtime }: { receipt?: Receipt; runtime: WorkspaceRuntime }) {
  ...
  return <section className="approval-receipt" aria-label="Latest action receipt">
    ...
    <p><strong>{receipt.action.replaceAll('-', ' ')}</strong> · {receipt.summary}<small>Actor: {receipt.actor} · Board V{String(receipt.version).padStart(2, '0')} · {new Date(receipt.timestamp).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</small></p>
