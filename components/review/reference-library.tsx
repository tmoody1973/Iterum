...
export function ReferenceLibrary({ snapshot, runtime }: { snapshot: WorkspaceState; runtime: WorkspaceRuntime }) {
  ...
  const entries = useMemo(() => [...snapshot.boardItems.filter((item) => item.kind === 'reference' || item.kind === 'agent-addition').map(entryFromBoard), ...snapshot.proposals.filter((proposal) => proposal.status === 'pending').map(entryFromProposal)], [snapshot])
