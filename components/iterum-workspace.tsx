...
import { BoardSemanticOutline } from './mechanical/board-outline'
import { LayersPanel } from './mechanical/layers-panel'
...
import { OrganizeStudio } from './organize/organize-studio'
...
export function IterumWorkspace({ runtime, showPendingProposalPreview = true, reviewOnMount = false, initialPreviewLayoutProposalId, projectController, projectStatus, reviewerGrant }: { runtime?: WorkspaceRuntime; showPendingProposalPreview?: boolean; reviewOnMount?: boolean; initialPreviewLayoutProposalId?: string; projectController?: ProjectController; projectStatus?: ProjectSaveStatus; reviewerGrant?: ReviewerGrantSession }) {
  ...
  const boardDisplayMode = useUiStore((state) => state.boardDisplayMode)
  ...
  const pendingProposal = showPendingProposalPreview ? snapshot.proposals.find((proposal) => proposal.id === 'proposal-resin' && proposal.status === 'pending') : undefined
  ...
  useEffect(() => {
    ...
    const syncDrawerForViewport = () => {
      ...
      setReviewDrawerOpen(reviewOnMount || media.matches)
      if (reviewOnMount) {
        setActiveRightTab('review')
        if (initialPreviewLayoutProposalId) useUiStore.getState().setPreviewLayoutProposal(initialPreviewLayoutProposalId)
      }
    }
  ...
  }, [initialPreviewLayoutProposalId, reviewOnMount, setActiveRightTab, setBriefDrawerOpen, setReviewDrawerOpen])
  ...
  const closeMechanicalPanel = () => {
    const previousMode = activeWorkspaceMode
    setActiveWorkspaceMode('mechanical')
    requestAnimationFrame(() => document.getElementById(`workspace-mode-${previousMode}`)?.focus())
  }

  return (
    <div className={`workspace is-${boardDisplayMode}${isBriefDrawerOpen ? ' is-brief-drawer-open' : ''}${isReviewDrawerOpen ? ' is-review-drawer-open' : ''}`} data-testid="iterum-workspace">
      <TopToolbar
      ...
      <div className="desk-zones">
        ...
        <main className="working-mechanical" aria-label="Working mechanical">
          ...
          {activeTool === 'organize' && <OrganizeStudio snapshot={snapshot} runtime={workspaceRuntime} selectedItemId={selectedBoardItemId} onClose={() => setActiveTool('select')} onProposed={(proposalId) => { useUiStore.getState().setPreviewLayoutProposal(proposalId); setActiveTool('select'); setActiveRightTab('review'); setReviewDrawerOpen(true) }} />}
          <div className="ruler ruler-top" aria-hidden="true" />
          <div className={`mechanical-stage${activeWorkspaceMode === 'mechanical' ? '' : ' is-panel-open'}`}>
            <section className="pasteboard" aria-label={`${snapshot.campaign.name} campaign board`}>
              <div className="mechanical-canvas-host" ref={canvasHost} aria-hidden="true">
                {canvasSize.width > 0 && canvasSize.height > 0 && <MechanicalCanvas snapshot={snapshot} runtime={workspaceRuntime} width={canvasSize.width} height={canvasSize.height} selectedId={selectedBoardItemId} onSelect={selectBoardItem} presentation={boardDisplayMode === 'presentation'} proposalPreview={pendingProposal ? { proposal: pendingProposal, position: pendingPlacement } : undefined} layoutPreview={layoutPreview} />}
              </div>
            ...
            {activeWorkspaceMode === 'layers' ? <LayersPanel snapshot={snapshot} runtime={workspaceRuntime} selectedId={selectedBoardItemId} onSelect={selectBoardItem} onClose={closeMechanicalPanel} /> : <BoardSemanticOutline snapshot={snapshot} />}
            {activeWorkspaceMode === 'history' && <ActionHistory snapshot={snapshot} runtime={workspaceRuntime} onClose={closeMechanicalPanel} projectController={projectController} />}
          </div>
      ...
      <BottomModeBar version={snapshot.version} campaignName={snapshot.campaign.name} projectStatus={projectStatus} />
    </div>
  )
}
