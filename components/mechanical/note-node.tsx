...
export function NoteNode({ item, selected, panEnabled, onSelect, onDragEnd, nodeRef }: {
  ...
  return <Group id={item.id} ref={nodeRef} x={item.position.x} y={item.position.y} width={item.width} height={item.height} draggable={!item.locked && !panEnabled} onClick={onSelect} onTap={onSelect} onDragEnd={(event) => onDragEnd(event.target as Konva.Group, { x: event.target.x(), y: event.target.y() })}>
    ...
    <Text x={16} y={item.height - 22} width={item.width - 30} text={`${item.groupLabel ? `${item.groupLabel.toUpperCase()} · ` : ''}${item.locked ? 'LOCKED' : 'SELECT · MOVE · RESIZE'}`} fontFamily="IBM Plex Mono" fontSize={7} fill={tone.accent} />
  </Group>
}
