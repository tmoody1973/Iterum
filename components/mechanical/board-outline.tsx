...
export function BoardOutline({ snapshot, runtime, selectedId, onSelect }: { snapshot: WorkspaceState; runtime: WorkspaceRuntime; selectedId: string | null; onSelect: (id: string) => void }) {
  ...
  return <section className="board-outline" aria-label="Board outline">
    ...
    <p className="board-outline-help">Select an item. Arrow keys move unlocked items by 10 px.</p>
    <ol>
      {snapshot.boardItems.map((item) => <li key={item.id} className={selectedId === item.id ? 'is-selected' : ''}>
        <button type="button" aria-label={`Select ${item.title}`} aria-pressed={selectedId === item.id} onClick={() => onSelect(item.id)} onKeyDown={(event) => onKeyDown(event, item)}>
          ...
          <span className="outline-actions">{item.locked ? <><LockKeyhole aria-hidden="true" />Locked · inspect/select only</> : <><Move aria-hidden="true" />Arrow keys move · Delete available</>}</span>
        </button>{item.locked && <button type="button" className="outline-delete" disabled aria-label={`Delete ${item.title}`}>Delete unavailable</button>}
      </li>)}
    </ol>
  </section>
}
