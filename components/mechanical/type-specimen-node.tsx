...
export function TypeSpecimenNode({ item, direction, selected, panEnabled, presentation = false, onSelect, onDragEnd, nodeRef }: {
  item: BoardItem
  ...
  const specimen = isHeadline
    ...
    : item.title.replace(/^Body:\s*/i, '').replace(/ \/ /g, '\n')
  ...
  return <Group id={item.id} ref={nodeRef} x={item.position.x} y={item.position.y} width={item.width} height={item.height} draggable={!item.locked && !panEnabled} onClick={onSelect} onTap={onSelect} onDragEnd={(event) => onDragEnd(event.target as Konva.Group, { x: event.target.x(), y: event.target.y() })}>
    ...
    {!presentation && <Text x={12} y={11} width={item.width - 24} text={`TYPE SYSTEM / ${isHeadline ? 'HEADLINE' : 'BODY'}`} fontFamily="IBM Plex Mono" fontSize={8} fill="#a05040" />}
    <Text x={12} y={presentation ? 18 : 30} width={item.width - 24} height={item.height - (presentation ? 36 : 64)} text={specimen} fontFamily={family} fontSize={isHeadline ? 34 : 16} lineHeight={isHeadline ? .86 : 1.35} fill="#171717" wrap="word" />
    {!presentation && <>
