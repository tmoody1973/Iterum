...
import { createReviewerTools, type WebMcpAgentSession } from './register-review-tools'
...
export async function registerIterumTools(runtime: WorkspaceRuntime, controller = new AbortController(), viewportController?: BoardViewportController, reviewUi?: ReviewUiController, presentationUi?: PresentationUiController, projectController?: ProjectController, agentSession?: WebMcpAgentSession): Promise<RegisteredTools | null> {
  if (!document.modelContext) return null
  if (agentSession?.role === 'creative') {
    const baseRuntime = runtime
    runtime = {
      ...baseRuntime,
      dispatch: (command) => baseRuntime.dispatch(command.actor === 'agent' ? { ...command, proposerSessionId: agentSession.sessionId } : command),
    }
  }
  let tools: WebMCPTool[] = [
    {
      ...
      execute: (input) => { const state = runtime.getSnapshot(); if (!validContext(state, input)) return invalid(state, 'campaignId and boardId are the only accepted fields for the open campaign.'); return success(state, { campaign: state.campaign, creativeRoutes: state.creativeRoutes, placementPolicy: state.placementPolicy, colorPalette: state.colorPalette, typeDirection: state.typeDirection, typeProposals: state.typeProposals, layoutProposals: state.layoutProposals, proposals: state.proposals, receipts: state.receipts.slice(0, 8), webMcpSession: agentSession ? { role: agentSession.role, sessionId: agentSession.sessionId, independentReviewer: agentSession.role === 'reviewer' } : null, persistence: projectController ? projectController.getStatus() : null, imageGeneration: projectController?.imageGeneration ? { model: 'gpt-image-2', tools: ['generate_image_candidates', 'edit_image_candidate', 'generate_campaign_applications', 'get_image_generation_run'], approvalBoundary: 'Every generated output enters Review before it can become board content.' } : null, productBoundary: projectController ? 'Convex cloud persistence, autosave, immutable snapshots, recovery, and review-first image generation are active. Presentation export remains future work.' : 'This fixture is intentionally session-only. Open a cloud project for persistence and recovery.' }, `Read ${state.campaign.name} at board version ${state.version}.`) },
    },
    {
  ...
  if (agentSession?.role === 'reviewer') {
    const safeReadTools = tools.filter((tool) => tool.annotations?.readOnlyHint === true)
    tools = projectController?.reviewerGrants ? [...safeReadTools, ...createReviewerTools(runtime, agentSession, projectController)] : safeReadTools
  } else if (projectController?.imageGeneration) {
    tools.push(...createImageGenerationTools(runtime, projectController, reviewUi))
  }
